...
- KPI cards for today's orders, revenue, new customers, returning customers,
  subscription orders, AOV, and a separately loaded live WooCommerce
  `processing` total. The live `Open Orders` card appears first and shows
  loading or unavailable instead of a false zero.
