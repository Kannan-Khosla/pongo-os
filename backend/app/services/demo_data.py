...
def seed_demo_data(engine) -> None:
    ...
    with Session(engine) as db:
        ...
        receiving = InventoryLocation(client="Reference Pet Supplies", warehouse="Main Warehouse", location_code="RECEIVING", location_name="Receiving", zone="Inbound", is_default=False, active=True)
        aisle_a = InventoryLocation(client="Reference Pet Supplies", warehouse="Main Warehouse", location_code="A-01", location_name="Aisle A / Shelf 1", zone="Food", aisle="A", shelf="1", is_default=True, active=True)
        aisle_b = InventoryLocation(client="Reference Pet Supplies", warehouse="Main Warehouse", location_code="B-02", location_name="Aisle B / Shelf 2", zone="Supplies", aisle="B", shelf="2", is_default=False, active=True)
        db.add_all([receiving, aisle_a, aisle_b])
        db.flush()

        item_specs = [
            ("DEMO-1001", "Reference Adult Dog Food – Chicken", "Dog Food", "Northstar Pets", "24", "2", "54.99", "31.50", aisle_a),
            ("DEMO-1002", "Reference Grain-Free Cat Food", "Cat Food", "Meadow & Co.", "14", "1", "38.99", "21.25", aisle_a),
            ("DEMO-1003", "Reference Dental Chews – Medium", "Treats", "BrightPaws", "6", "2", "18.49", "9.10", aisle_b),
            ("DEMO-1004", "Reference Lavender Cat Litter", "Cat Litter", "CleanHome", "3", "0", "22.99", "13.40", aisle_b),
            ("DEMO-1005", "Reference Rope Tug Toy", "Toys", "Playful Pack", "18", "0", "12.99", "5.25", aisle_b),
            ("DEMO-1006", "Reference Salmon Training Treats", "Treats", "Northstar Pets", "0", "0", "10.99", "4.60", aisle_a),
            ("DEMO-1007", "Reference Stainless Steel Bowl", "Bowls", "Everyday Pet", "11", "0", "16.99", "7.75", aisle_b),
            ("DEMO-1008", "Reference Puppy Pads – 50 Pack", "Training", "CleanHome", "9", "0", "29.99", "16.20", aisle_b),
        ]
        ...
        for index, (sku, name, category, brand, stock, allocated, price, cost, location) in enumerate(item_specs, start=1):
            ...
            db.add(row)
            db.add(WooItemMapping(item_id=item.id, woo_product_id=item.woo_product_id, woo_sku=sku, woo_name=name, mapping_source="demo", confidence=100, active=True, note="Reference storefront mapping"))
            items.append(item)
            item_locations.append(row)
        db.flush()

        addresses = [
            ("Reference Customer North", "7424 118 Avenue NW", "T5B 4M9"),
            ("Reference Customer West", "8882 170 Street NW", "T5T 4J2"),
            ("Reference Customer Central", "1 Sir Winston Churchill Square", "T5J 2R7"),
            ("Reference Customer South", "7000 143 Street NW", "T6H 4P3"),
            ("Reference Customer East", "10220 104 Avenue NW", "T5J 0H6"),
        ]
        orders: list[Order] = []
        ...
        for index, (customer, address, postal) in enumerate(addresses, start=1):
            ...
            order = Order(
                ...
                customer_first_name="Reference",
                customer_last_name=customer.removeprefix("Reference Customer "),
                ...
                payment_method_title="Reference card",
                ...
                raw_woo_payload={"customer_note": "Reference delivery note: leave at reception."},
            )
            db.add(order)
            db.flush()
            item = items[index - 1]
        ...
        receipt = Receipt(receipt_number="DEMO-RCV-0001", receipt_type="direct", status="posted", source="demo", client="Reference Pet Supplies", warehouse="Main Warehouse", reference_number="DEMO-SHIPMENT-01", created_by="demo-system", received_by="Reference Warehouse", received_date=date.today() - timedelta(days=2), received_at=now - timedelta(days=2), committed_at=now - timedelta(days=2), notes="Reference receiving record")
        db.add(receipt)
        db.flush()
        db.add(ReceiptItem(receipt_id=receipt.id, inventory_item_id=items[0].id, inventory_location_id=aisle_a.id, inventory_item_location_id=item_locations[0].id, line_status="posted", sku=items[0].sku, description=items[0].description, quantity=Decimal("12"), quantity_received=Decimal("12"), uom="Each", unit_cost=items[0].unit_cost, unit_cost_total=items[0].unit_cost * 12, sales_price=items[0].sales_price, brand=items[0].brand, client="Reference Pet Supplies", warehouse="Main Warehouse", inventory_location_name="A-01", default_location="A-01", received_date=date.today() - timedelta(days=2), po_or_receipt_number=receipt.receipt_number, name=items[0].description, notes="Reference delivery"))
        db.add(StockMovement(inventory_item_id=items[0].id, inventory_location_id=aisle_a.id, inventory_item_location_id=item_locations[0].id, sku=items[0].sku, barcode=items[0].barcode, movement_type=MovementType.receiving, quantity_change=Decimal("12"), old_stock=Decimal("12"), new_stock=Decimal("24"), warehouse="Main Warehouse", inventory_location_name="A-01", old_location_stock=Decimal("12"), new_location_stock=Decimal("24"), old_item_stock=Decimal("12"), new_item_stock=Decimal("24"), movement_source="demo", reference_number=receipt.receipt_number, unit_cost=items[0].unit_cost, reason="Reference receiving", reference_type="receipt", reference_id=receipt.id, created_by="demo-system", created_at=now - timedelta(days=2)))

        allocation = Allocation(allocation_number="DEMO-ALC-0001", status="posted", allocation_type="single_order", order_id=orders[1].id, woo_order_id=orders[1].woo_order_id, woo_order_number=orders[1].woo_order_number, notes="Reference allocation", created_by="demo-system", posted_at=now - timedelta(hours=8))
        db.add(allocation)
        db.flush()
        db.add(AllocationLine(allocation_id=allocation.id, order_id=orders[1].id, order_line_id=order_lines[1].id, item_id=items[1].id, inventory_item_location_id=item_locations[1].id, sku=items[1].sku, barcode=items[1].barcode, description=items[1].description, warehouse="Main Warehouse", inventory_location="A-01", quantity_ordered=Decimal("2"), quantity_previously_allocated=Decimal("0"), quantity_to_allocate=Decimal("2"), quantity_allocated_after=Decimal("2"), in_stock_before=Decimal("14"), allocated_before=Decimal("0"), sellable_before=Decimal("14"), allocated_after=Decimal("2"), sellable_after=Decimal("12"), shortage_quantity=Decimal("0"), status="allocated", notes="Reference allocation", auto_allocated=False, allocation_source="demo"))

        pick = Pick(pick_number="DEMO-PICK-0001", status="posted", pick_type="single_order", order_id=orders[3].id, woo_order_id=orders[3].woo_order_id, woo_order_number=orders[3].woo_order_number, notes="Reference pick", created_by="demo-system", posted_at=now - timedelta(hours=4))
        db.add(pick)
        db.flush()
        db.add(PickLine(pick_id=pick.id, order_id=orders[3].id, order_line_id=order_lines[3].id, item_id=items[3].id, inventory_item_location_id=item_locations[3].id, sku=items[3].sku, barcode=items[3].barcode, description=items[3].description, warehouse="Main Warehouse", inventory_location="B-02", quantity_ordered=Decimal("2"), quantity_allocated=Decimal("2"), quantity_previously_picked=Decimal("0"), quantity_to_pick=Decimal("2"), quantity_picked_after=Decimal("2"), remaining_to_pick=Decimal("0"), quantity_stock_reduced=Decimal("2"), stock_reduced_at=now - timedelta(hours=4), idempotency_key="demo-pick", status="picked", notes="Reference pick"))

        fulfillment = Fulfillment(fulfillment_number="DEMO-FUL-0001", status="posted", fulfillment_type="single_order", order_id=orders[4].id, woo_order_id=orders[4].woo_order_id, woo_order_number=orders[4].woo_order_number, notes="Reference completed order", created_by="demo-system", posted_at=now - timedelta(days=1))
        db.add(fulfillment)
        db.flush()
        db.add(FulfillmentLine(fulfillment_id=fulfillment.id, order_id=orders[4].id, order_line_id=order_lines[4].id, item_id=items[4].id, inventory_item_location_id=item_locations[4].id, sku=items[4].sku, barcode=items[4].barcode, description=items[4].description, warehouse="Main Warehouse", inventory_location="B-02", quantity_ordered=Decimal("2"), quantity_allocated=Decimal("2"), quantity_picked=Decimal("2"), quantity_previously_fulfilled=Decimal("0"), quantity_to_fulfill=Decimal("2"), unit_cost=items[4].unit_cost, quantity_fulfilled_after=Decimal("2"), remaining_to_fulfill=Decimal("0"), in_stock_before=Decimal("20"), allocated_before=Decimal("2"), sellable_before=Decimal("18"), in_stock_after=Decimal("18"), allocated_after=Decimal("0"), sellable_after=Decimal("18"), status="fulfilled", notes="Reference fulfillment"))

        route = Route(route_number="DEMO-ROUTE-001", status="finalized", route_name="Reference Edmonton Run", route_date=date.today(), driver_name="Reference Driver", vehicle_name="Reference Van", notes="Reference route", start_address="100 Reference Warehouse Road, Edmonton, AB", end_address="100 Reference Warehouse Road, Edmonton, AB", total_stops=2, total_distance=Decimal("18.50"), estimated_duration=55, estimated_duration_minutes=55, map_provider="google_maps", optimization_status="preview", created_by="demo-system", finalized_at=now - timedelta(hours=1))
        db.add(route)
        db.flush()
        for sequence, order in enumerate(orders[:2], start=1):
            db.add(RouteStop(route_id=route.id, stop_sequence=sequence, order_id=order.id, woo_order_id=order.woo_order_id, woo_order_number=order.woo_order_number, stop_number=sequence, customer_name=order.customer_name, customer_email=order.customer_email, customer_phone=order.customer_phone, shipping_summary=order.shipping_summary, delivery_notes="Reference delivery stop", local_status=order.local_status, stop_status="planned", address_1=order.shipping_address_1, city=order.shipping_city, state=order.shipping_state, country=order.shipping_country, zip=order.shipping_zip, phone=order.shipping_phone, geocode_status="not_requested", optimized_sequence=sequence, notes="Reference data"))

        db.add(WooCommerceSyncRun(sync_type="products", status="completed", started_at=now - timedelta(minutes=20), completed_at=now - timedelta(minutes=19), created_by="demo-system", total_remote_records=len(items), created_count=0, updated_count=0, matched_count=len(items), skipped_count=0, conflict_count=0, error_count=0, notes="Reference WooCommerce catalog sync"))
        db.commit()
