...
const OUTCOME_ICONS = { add_items: PackagePlus, update_items: Pencil, starting_inventory: Warehouse };
...
function OutcomeStep({ schema, outcome, setOutcome, onContinue }) {
  return (
    <section className="import-stage" aria-labelledby="outcome-title">
      ...
      <div className="import-assurance"><ShieldCheck size={19} /><span><strong>Inventory-safe by design.</strong> Item-detail files cannot contain on-hand, allocated, available, or movement-history fields.</span></div>
  ...
  return (
    <section className="import-stage" aria-labelledby="upload-title">
      ...
      <div className="template-banner">
        ...
        <div><a className="import-secondary-button" href={downloadHref(outcome)}><Download size={17} /> Download template</a>{outcome === 'update_items' && <a className="import-text-link" href={downloadHref(outcome, true)}>Export editable existing items</a>}</div>
      </div>
      ...
      <div className="import-stage-actions"><button className="import-quiet-button" onClick={onBack} type="button"><ArrowLeft size={17} /> Back</button><button className="import-primary-button" disabled={!file || busy} onClick={onUpload} type="button">{busy ? 'Reading and checking…' : 'Upload and match columns'} <ArrowRight size={17} /></button></div>
    </section>
  );
}
...
function ReviewStep({ schema, preview, setPreview, onContinue, onBack, busy, setBusy, setError }) {
  ...
  return (
    <section className="import-stage wide" aria-labelledby="review-title">
      ...
      <div className="import-stage-actions"><button className="import-quiet-button" onClick={onBack} type="button"><ArrowLeft size={17} /> Back</button><button className="import-primary-button" disabled={!preview.summary?.ready_count || busy} onClick={onContinue} type="button">Review import <ArrowRight size={17} /></button></div>
...
function ResultsStep({ preview, onNewImport }) {
  ...
  return (
    <section className="import-stage result-stage" aria-labelledby="result-title">
      ...
      <div className="result-reference"><span>Import job #{result.import_job_id}</span><span>{formatNumber(result.duration_ms)} ms</span>{result.starting_units > 0 && <span>{formatNumber(result.starting_units)} starting units</span>}</div>
...
export function ItemImportWorkspace({ initialPreviewId = '' }) {
  ...
  return (
    <section className="item-import-workspace">
      <PageIntro title="Import items" description={intro ? `${intro.label}: ${intro.description}` : 'A guided, inventory-safe workspace for adding items, updating details, or recording starting inventory.'} onCancel={preview || step > 1 ? cancel : null} />
