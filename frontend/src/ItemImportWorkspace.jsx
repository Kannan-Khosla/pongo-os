...
const OUTCOME_ICONS = { add_items: PackagePlus, update_items: Pencil, repair_items: Wrench, update_stock: RefreshCw, starting_inventory: Warehouse };
const PREVIEW_KEY = 'pongo.item-import.preview';
const REPAIR_POLL_DELAYS_MS = [500, 1000, 2000, 4000, 5000];

const OUTCOME_LABELS = {
  add_items: 'Add new items',
  update_items: 'Update item details',
  repair_items: 'Repair item data & locations',
  update_stock: 'Override stock levels',
  starting_inventory: 'Set starting inventory',
};
...
function isStockSensitiveOutcome(outcome) {
  return outcome === 'update_stock' || outcome === 'repair_items';
}
...
function SummaryCards({ summary, outcome }) {
  const cards = outcome === 'starting_inventory'
    ? [
      ...
      ? [
        ['Will repair', summary?.update_count ?? summary?.ready_count],
        ['Locations consolidated', summary?.consolidate_count],
        ['Location moves deferred', summary?.deferred_location_count],
        ['Units relocated', summary?.units_relocated],
        ['Blocked', summary?.blocking_count],
...
function OutcomeStep({ schema, outcome, setOutcome, onContinue }) {
  return (
    <section className="import-stage" aria-labelledby="outcome-title">
      ...
      <div className="import-assurance"><ShieldCheck size={19} /><span><strong>Inventory-safe by design.</strong> Stock-sensitive imports show exact before-and-after values. Repairs move only unallocated units, defer reserved units, and never change product totals or WooCommerce stock.</span></div>
  ...
  return (
    <section className="import-stage" aria-labelledby="upload-title">
      ...
      <div className="template-banner">
        ...
        <div><a className="import-secondary-button" href={downloadHref(outcome)}><Download size={17} /> Download template</a>{outcome === 'update_items' && <a className="import-text-link" href={downloadHref(outcome, true)}>Export editable existing items</a>}{outcome === 'repair_items' && <a className="import-text-link" href={downloadHref(outcome, true)}>Export repair-ready catalog</a>}{outcome === 'update_stock' && <a className="import-text-link" href={downloadHref(outcome, true)}>Export editable current stock</a>}</div>
      </div>
    ...
    <div className="item-import-workspace import-overlay-root">
      <div className="import-drawer-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
        <aside className="import-row-drawer" ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="row-editor-title" aria-describedby={row.issues?.length ? 'row-editor-issues' : undefined}>
          ...
          <div className="row-editor-fields">{fields.map((field, index) => <label key={field.key}><span>{field.label}</span><input autoFocus={index === 0} readOnly={field.editable === false} type={['decimal', 'integer'].includes(field.type) ? 'number' : 'text'} value={values[field.key]} onChange={(event) => setValues((current) => ({ ...current, [field.key]: event.target.value }))} /></label>)}</div>
...
function ReviewStep({ schema, preview, setPreview, onContinue, onBack, busy, setBusy, setError }) {
  ...
  const isRepairImport = preview.outcome === 'repair_items';
  const isGuardedImport = isStockSensitiveOutcome(preview.outcome);
  ...
  const issueCount = isGuardedImport
  ...
  const importBlocked = isGuardedImport && stockImportBlocked(preview.summary);
  const rowFilters = isRepairImport
    ? [['', 'All'], ['will_update', 'Will repair'], ['no_changes', 'Already matches'], ['needs_attention', 'Needs attention'], ['duplicate', 'Duplicates'], ['unmatched', 'Not found'], ['blocked', 'Blocked']]
    : isStockImport
  ...
  const reviewDescription = isRepairImport
    ? 'Nothing has changed yet. Brand and unit cost updates, the Store/default target, and every safe unallocated-stock move are validated together. Reserved units stay at their source location and are reported as deferred.'
    : isStockImport
      ? `Nothing has changed yet. ${formatNumber(preview.summary?.source_row_count)} CSV rows were combined into ${formatNumber(preview.summary?.sku_count)} SKU totals; distinct CSV location rows are summed and do not reassign Pongo locations.`
      : 'Nothing has been imported. Correct rows here or exclude them from this run.';
  const noticeTitle = importBlocked
    ? `${formatNumber(issueCount)} issue(s) block this ${isRepairImport ? 'repair' : 'stock import'}`
    : issueCount
      ? `${formatNumber(issueCount)} row(s) need a decision`
      : isRepairImport
        ? 'All item repairs are ready'
        : isStockImport ? 'All matched SKUs are ready' : 'Every row is ready';
  const noticeDetail = importBlocked
    ? isRepairImport ? 'Fix every blocking issue. No repair rows will import separately.' : `Fix the blocking issue(s). ${formatNumber(skippedCount)} SKU(s) not found in Pongo will be safely skipped.`
    : issueCount
      ? 'Fix the values, exclude those rows, or continue with the valid rows.'
      : isRepairImport
        ? `${formatNumber(preview.summary?.consolidate_count)} item location(s) will consolidate; ${formatNumber(preview.summary?.deferred_location_count)} reserved-stock location move(s) will be deferred.`
        : isStockImport ? `Every matched difference will be applied together. ${formatNumber(skippedCount)} SKU(s) not found in Pongo will stay unchanged.` : 'Review the proposed changes before confirming.';
  return (
    <section className="import-stage wide" aria-labelledby="review-title">
      <div className="import-stage-heading"><span>Step 4</span><h3 id="review-title">Review and fix issues</h3><p>{reviewDescription}</p></div>
      <SummaryCards summary={preview.summary} outcome={preview.outcome} />
      <div className={`import-notice ${importBlocked || issueCount ? 'warning' : 'success'}`}>{importBlocked || issueCount ? <AlertTriangle size={18} /> : <CheckCircle2 size={18} />}<span><strong>{noticeTitle}</strong><small>{noticeDetail}</small></span></div>
      <div className="review-toolbar"><div className="import-segmented" aria-label="Row status filter">{rowFilters.map(([value, label]) => <button className={state === value ? 'active' : ''} key={label} onClick={() => setState(value)} type="button">{label}</button>)}</div><form onSubmit={(event) => { event.preventDefault(); setSearch(searchDraft); }}><Search size={16} /><input aria-label="Search preview rows" placeholder={isRepairImport ? 'Search SKU or product name' : 'Search SKU, barcode, name'} value={searchDraft} onChange={(event) => setSearchDraft(event.target.value)} /></form></div>
      <div className="review-table-wrap"><table className="review-table"><thead><tr><th>Row</th><th>{isRepairImport ? 'Pongo ID / SKU' : 'SKU / barcode'}</th><th>Product</th><th>Result</th><th>Issue or proposed change</th><th><span className="sr-only">Actions</span></th></tr></thead><tbody>{rowsData.rows.map((row) => <tr key={row.id} className={row.excluded ? 'excluded' : ''}><td>{row.row_number}</td><td><strong>{isRepairImport ? row.existing_item_id ?? row.normalized_data?.item_id ?? '—' : row.sku || '—'}</strong><small>{isRepairImport ? row.sku || 'No SKU' : row.barcode || ''}</small></td><td>{row.product_name || '—'}</td><td><StatusPill state={row.state} /></td><td>{row.issues?.[0] ? <span className="row-message issue">{row.issues[0].message}</span> : <span className="row-change-summary">{Object.values(row.proposed_changes || {}).slice(0, 2).map((change) => change.label).join(', ') || 'No data changes'}{Object.keys(row.proposed_changes || {}).length > 2 && ` +${Object.keys(row.proposed_changes).length - 2}`}</span>}</td><td><div className="row-actions"><button aria-label={`Edit row ${row.row_number}`} onClick={() => setEditing(row)} type="button"><Pencil size={16} /></button>{!isRepairImport && (!isStockImport || row.excluded) && <button aria-label={`${row.excluded ? 'Include' : 'Exclude'} row ${row.row_number}`} onClick={() => updateRow(row, { excluded: !row.excluded })} type="button">{row.excluded ? <RotateCcw size={16} /> : <X size={16} />}</button>}</div></td></tr>)}</tbody></table>{!rowsData.rows.length && <div className="review-empty">No rows match this filter.</div>}</div>
      ...
      <div className="import-stage-actions"><button className="import-quiet-button" onClick={onBack} type="button"><ArrowLeft size={17} /> Back</button><button className="import-primary-button" disabled={busy || importBlocked || (!isGuardedImport && !preview.summary?.ready_count)} onClick={onContinue} type="button">{isRepairImport ? 'Review repair' : 'Review import'} <ArrowRight size={17} /></button></div>
...
function ConfirmStep({ preview, rowsData, onLoadPage, onBack, onCommit, busy }) {
  ...
  const confirmationWord = isRepairImport ? 'REPAIR' : preview.outcome === 'starting_inventory' ? 'START' : isStockImport && !noStockChanges ? 'STOCK' : '';
  const importBlocked = isGuardedImport && stockImportBlocked(preview.summary);
  const enabled = confirmed && (!confirmationWord || typed === confirmationWord) && !importBlocked;
  const rows = rowsData.rows || [];
  const proposedChanges = rows.flatMap((row) => Object.values(row.proposed_changes || {}).map((change) => ({ ...change, row_id: row.id, identity: isRepairImport ? row.existing_item_id ?? row.normalized_data?.item_id ?? row.sku : row.sku })));
  const confirmDescription = isRepairImport
    ? `${formatNumber(readyCount)} matched item(s) will repair together in one transaction: ${formatNumber(preview.summary?.consolidate_count)} location(s) consolidate, ${formatNumber(preview.summary?.units_relocated)} unallocated unit(s) move, and ${formatNumber(preview.summary?.deferred_location_count)} reserved-stock location move(s) remain deferred. Product totals and WooCommerce stock do not change.`
    : isStockImport
      ? `${formatNumber(preview.summary?.update_count)} matched SKU(s) will update together in one transaction. ${formatNumber(preview.summary?.skipped_count)} SKU(s) not found in Pongo will be skipped. Distinct CSV location rows are summed by SKU; Pongo items absent from this file remain unchanged.`
      : 'The preview is saved. Every source row is available page by page; if an item changed since validation, commit will stop and ask you to refresh.';
  const confirmationTitle = isRepairImport
    ? 'I reviewed every item repair and deferred reserved-stock move'
    : isStockImport ? 'I reviewed the matched updates and skipped SKUs' : 'I reviewed the outcome, valid row count, and applicable change pages';
  const confirmationDetail = isRepairImport
    ? 'Every repair applies together; any blocking validation or stale-data issue stops all changes. Total stock and WooCommerce stock stay unchanged.'
    : isStockImport ? 'Matched changes apply together; any blocking validation or stale-data issue stops every stock change.' : 'I understand this action creates an audit record and cannot silently overwrite newer data.';
  return (
    <section className="import-stage wide" aria-labelledby="confirm-title">
      <div className="import-stage-heading"><span>Step 5</span><h3 id="confirm-title">Review changes before import</h3><p>{confirmDescription}</p></div>
      <SummaryCards summary={preview.summary} outcome={preview.outcome} />
      ...
      <div className="change-table-wrap"><table className="change-table"><thead><tr><th>{isRepairImport ? 'Pongo ID / SKU' : 'SKU'}</th><th>Field</th><th>Current value</th><th>New value</th></tr></thead><tbody>{proposedChanges.map((change) => <tr key={`${change.row_id}-${change.field}`}><td>{change.identity || '—'}</td><td>{change.label}</td><td>{String(change.before ?? 'Blank')}</td><td>{String(change.after ?? 'Blank')}</td></tr>)}</tbody></table>{!proposedChanges.length && <div className="review-empty">No field-level changes on this page.</div>}</div>
      <div className="review-pagination"><span>{formatNumber(rowsData.total || 0)} {isRepairImport ? 'item repair(s)' : isStockImport ? 'SKU total(s)' : 'source row(s)'} available for review</span><div><button disabled={rowsData.page <= 1 || busy} onClick={() => onLoadPage(rowsData.page - 1)} type="button">Previous</button><span>Page {rowsData.page} of {Math.max(rowsData.total_pages, 1)}</span><button disabled={rowsData.page >= rowsData.total_pages || busy} onClick={() => onLoadPage(rowsData.page + 1)} type="button">Next</button></div></div>
      ...
      {importBlocked && <div className="import-notice warning"><AlertTriangle size={18} /><span><strong>Import blocked</strong><small>Return to review and fix every row. No {isRepairImport ? 'repair' : 'stock'} rows will import separately.</small></span></div>}
      {!isGuardedImport && hasBlockingIssues(preview.summary) && <div className="import-notice warning"><AlertTriangle size={18} /><span><strong>Only valid rows will import</strong><small>Rows needing attention stay unchanged and will be available as a correction file in the results.</small></span></div>}
      {confirmationWord && <label className="typed-confirm"><span>Type <strong>{confirmationWord}</strong> to confirm this audited {isRepairImport ? 'item repair' : 'stock change'}</span><input aria-label={`Type ${confirmationWord} to confirm`} autoComplete="off" value={typed} onChange={(event) => setTyped(event.target.value.toUpperCase())} /></label>}
      <label className="import-checkbox-card confirmation"><input checked={confirmed} onChange={(event) => setConfirmed(event.target.checked)} type="checkbox" /><span><strong>{confirmationTitle}</strong><small>{confirmationDetail}</small></span></label>
      <div className="import-stage-actions"><button className="import-quiet-button" onClick={onBack} type="button"><ArrowLeft size={17} /> Back</button><button className="import-primary-button" disabled={!enabled || busy} onClick={onCommit} type="button">{busy ? 'Importing safely…' : isRepairImport ? `Repair ${formatNumber(readyCount)} item${readyCount === 1 ? '' : 's'}` : preview.outcome === 'starting_inventory' ? 'Record starting inventory' : noStockChanges ? 'Finish — no stock changes' : isStockImport ? `Apply ${formatNumber(readyCount)} stock change${readyCount === 1 ? '' : 's'}` : `Import ${formatNumber(readyCount)} ready item${readyCount === 1 ? '' : 's'}`} <Check size={17} /></button></div>
    </section>
  );
}

function ResultsStep({ preview, repairJob, onRetry, onNewImport, busy }) {
  const result = repairJob || preview.result || {};
  const success = result.status === 'completed';
  const isRepairImport = result.outcome === 'repair_items' || preview.outcome === 'repair_items';
  const pending = isRepairImport && ['queued', 'running'].includes(result.status);
  if (pending) {
    return (
      <section className="import-stage result-stage" aria-labelledby="result-title" aria-live="polite">
        <div className="result-icon warning"><RefreshCw size={34} /></div>
        <div className="import-stage-heading"><span>Step 6</span><h3 id="result-title">{result.status === 'queued' ? 'Repair queued' : 'Repair in progress'}</h3><p>The repair is running as a durable background job. You can leave this page and return without starting it again.</p></div>
        <div className="result-reference"><span>Import job #{result.import_job_id}</span><span>No item or location changes are shown until the whole repair succeeds.</span></div>
        <div className="result-actions"><a className="import-secondary-button" href="#/items/imports"><History size={17} /> View history</a><a className="import-quiet-button" href="#items">Return to items</a></div>
      </section>
    );
  }
  if (isRepairImport && result.status === 'failed') {
    return (
      <section className="import-stage result-stage" aria-labelledby="result-title">
        <div className="result-icon warning"><AlertTriangle size={34} /></div>
        <div className="import-stage-heading"><span>Step 6</span><h3 id="result-title">Repair failed safely</h3><p>{result.message || 'The repair could not be completed.'} No item or location changes were saved.</p></div>
        <div className="result-reference"><span>Import job #{result.import_job_id}</span><span>The preview is open again for review and retry.</span></div>
        <div className="result-actions"><button className="import-primary-button" disabled={busy} onClick={onRetry} type="button">{busy ? 'Refreshing preview…' : 'Review and retry'}</button><a className="import-secondary-button" href="#/items/imports"><History size={17} /> View history</a></div>
      </section>
    );
  }
  const metrics = isRepairImport
    ? [
      ['Updated', result.updated_count],
      ['Unchanged', result.unchanged_count],
      ['Locations consolidated', result.consolidate_count],
      ['Location moves deferred', result.deferred_location_count],
      ['Units relocated', result.units_relocated],
  ...
  const resultTitle = success
    ? isRepairImport ? 'Repair completed' : 'Import completed'
    : isRepairImport ? 'Repair completed with attention needed' : 'Import completed with attention needed';
  return (
    <section className="import-stage result-stage" aria-labelledby="result-title">
      <div className={`result-icon ${success ? 'success' : 'warning'}`}>{success ? <CheckCircle2 size={34} /> : <AlertTriangle size={34} />}</div>
      <div className="import-stage-heading"><span>Step 6</span><h3 id="result-title">{resultTitle}</h3><p>The job is recorded in import history with its file, user, result, field changes, and errors.</p></div>
      <div className="result-metrics">{metrics.map(([label, value]) => <div key={label}><span>{label}</span><strong>{formatNumber(value)}</strong></div>)}</div>
      <div className="result-reference"><span>Import job #{result.import_job_id}</span><span>{formatNumber(result.duration_ms)} ms</span>{result.starting_units > 0 && <span>{formatNumber(result.starting_units)} starting units</span>}{result.stock_adjustment_id && <span>Stock adjustment #{result.stock_adjustment_id}</span>}{result.woo_stock_sync_job_id && <span>Woo stock sync #{result.woo_stock_sync_job_id} queued</span>}{result.outcome === 'update_stock' && <span>{formatNumber(result.stock_units_delta)} net stock change</span>}{isRepairImport && <span>Product totals and WooCommerce stock unchanged</span>}</div>
...
export function ItemImportWorkspace({ initialPreviewId = '', initialOutcome = '', onCommitted = null }) {
  ...
  useEffect(() => {
    ...
    requestJson(`/api/items/import/previews/${resumeId}`).then((saved) => {
      ...
      const repairRunning = saved.outcome === 'repair_items' && ['queued', 'running'].includes(saved.status) && saved.import_job_id;
      if (repairRunning) setRepairJob({ import_job_id: saved.import_job_id, preview_id: saved.preview_id, outcome: saved.outcome, status: saved.status });
      const nextStep = saved.status === 'committed' || repairRunning ? 6 : (saved.status === 'draft' ? 3 : 4);
      setStep(nextStep);
  ...
  useEffect(() => {
    const jobId = repairJob?.import_job_id;
    if (!jobId || !['queued', 'running'].includes(repairJob.status) || !preview?.preview_id) return undefined;
    let cancelled = false;
    let timer;
    let attempt = 0;
    const schedule = () => {
      const delay = REPAIR_POLL_DELAYS_MS[Math.min(attempt, REPAIR_POLL_DELAYS_MS.length - 1)];
      attempt += 1;
      timer = window.setTimeout(poll, delay);
    };
    const poll = async () => {
      try {
        const job = await requestJson(`/api/import-jobs/${jobId}`, undefined, 'The repair status could not be refreshed.');
        if (cancelled) return;
        if (['queued', 'running'].includes(job.status)) {
          setRepairJob((current) => ({ ...current, status: job.status }));
          setError('');
          schedule();
          return;
        }
        const result = job.result_json || { import_job_id: job.id || jobId, preview_id: job.preview_id || preview.preview_id, outcome: job.outcome || 'repair_items', status: job.status, message: job.message };
        ...
        setStep(6);
        setMaxStep(6);
        if (result.status === 'completed') await onCommitted?.();
      } catch (requestError) {
        if (cancelled) return;
        setError(`${requestError.message} The repair is still recorded as job #${jobId}; Pongo will keep checking.`);
        schedule();
      }
    };
    schedule();
  ...
  }, [repairJob?.import_job_id, repairJob?.status, preview?.preview_id]);
  ...
  async function commit() {
    ...
    setError('');
    try {
      const result = await requestJson(`/api/items/import/previews/${preview.preview_id}/commit`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ idempotency_key: newIdempotencyKey() }) }, 'The import could not be committed.');
      if (preview.outcome === 'repair_items' && ['queued', 'running'].includes(result.status)) {
        setPreview((current) => ({ ...current, status: 'running', import_job_id: result.import_job_id, result: null }));
        setRepairJob(result);
        setStep(6);
        setMaxStep(6);
        return;
  ...
  function reset() {
    ...
    setOutcome(''); setFile(null); setPreview(null); setConfirmRows([]); setRepairJob(null); setStep(1); setMaxStep(2); setError('');
  }
  ...
  async function retryRepair() {
    ...
    setError('');
    try {
      const saved = await requestJson(`/api/items/import/previews/${preview.preview_id}/revalidate`, { method: 'POST' }, 'The repair preview could not be refreshed.');
      remember(saved);
      setRepairJob(null);
      setConfirmRows({ rows: [], total: 0, page: 1, page_size: 25, total_pages: 0 });
      setStep(4);
      setMaxStep(saved.status === 'ready' ? 5 : 4);
    } catch (requestError) {
      setError(requestError.message);
    } finally {
      setBusy(false);
  ...
  const repairPending = ['queued', 'running'].includes(repairJob?.status);
  return (
    <section className="item-import-workspace" ref={workspaceRef}>
      <PageIntro title="Import items" description={intro ? `${intro.label}: ${intro.description}` : 'A guided workspace for adding items, updating details, overriding stock, or recording starting inventory.'} onCancel={!repairPending && (preview || step > 1) ? cancel : null} />
      ...
      {preview && step === 6 && <ResultsStep preview={preview} repairJob={repairJob} onRetry={retryRepair} onNewImport={reset} busy={busy} />}
    </section>
  );
}
...
export function ItemImportHistory({ onRolledBack = null }) {
  ...
  return (
    <section className="item-import-workspace history-page">
      ...
      <div className="history-toolbar"><a className="import-primary-button" href="#/items/import" onClick={() => sessionStorage.removeItem(PREVIEW_KEY)}><PackagePlus size={17} /> New import</a><label><span>Outcome</span><select value={outcome} onChange={(event) => setOutcome(event.target.value)}><option value="">All outcomes</option><option value="add_items">Add new items</option><option value="update_items">Update item details</option><option value="repair_items">Repair item data &amp; locations</option><option value="update_stock">Override stock levels</option><option value="starting_inventory">Set starting inventory</option></select></label><label><span>Status</span><select value={status} onChange={(event) => setStatus(event.target.value)}><option value="">All statuses</option><option value="completed">Completed</option><option value="completed_with_errors">Completed with errors</option><option value="failed">Failed</option></select></label><button className="import-secondary-button" disabled={busy} onClick={() => load(jobsPagination.page, jobsPagination.page_size)} type="button"><RefreshCw size={16} /> Refresh</button></div>
          ...
          {jobs.map((job) => (
            <tbody key={job.id} className="history-job-group">
              <tr>
                ...
                <td>{OUTCOME_LABELS[job.outcome] || job.outcome?.replaceAll('_', ' ')}</td>
                <td>{job.file_name || '—'}</td>
              ...
              {expanded === job.id && (
                <tr className="history-detail-row"><td colSpan="8"><div className="history-detail">
                  <div><strong>Job summary</strong><p>{formatNumber(job.created_rows)} created · {formatNumber(job.updated_rows)} updated · {formatNumber(job.unchanged_rows)} unchanged · {formatNumber(job.excluded_rows)} excluded</p><p>{OUTCOME_LABELS[job.outcome] || job.outcome?.replaceAll('_', ' ')} · run by {job.created_by || 'System'} · completed {formatDate(job.completed_at || job.created_at)}</p>{job.starting_units > 0 && <p>{formatNumber(job.starting_units)} starting units recorded</p>}{job.outcome === 'repair_items' && <p>{formatNumber(job.result_json?.consolidate_count)} locations consolidated · {formatNumber(job.result_json?.deferred_location_count)} moves deferred · {formatNumber(job.result_json?.units_relocated)} units relocated</p>}</div>
