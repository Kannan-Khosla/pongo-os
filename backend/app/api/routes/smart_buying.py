...
from app.services.smart_buying import export_purchase_draft, load_smart_buying_demo
...
@router.get("/demo")
def smart_buying_demo() -> dict:
    return load_smart_buying_demo()
...
def export_smart_buying_draft(format: Literal["csv", "pdf"], payload: SmartBuyingExportRequest) -> Response:
    ...
    content = csv_text if format == "csv" else tabular_pdf_bytes(
        csv_text, f"Smart Buying / {payload.po_number} / Local draft - not sent"
    )
    return Response(
        content=content,
